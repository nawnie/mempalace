#!/bin/bash
# MEMPALACE PRE-COMPACT HOOK â€” Emergency save before compaction
#
...
# event â€” compaction is always worth saving before. The hook itself
...
# not emit a ``decision: block`` to the hook protocol â€” the
...
# MEMPAL_DIR is an *additional*, optional target for project files â€” it
...
# MEMPAL_PYTHON (explicit override) â†’ $(command -v python3) â†’ bare python3.
...
if [ -n "$INPUT" ] && [ "$_MEMPAL_PARSE_MARKER" != "__MEMPAL_PARSE_OK__" ]; then
    ...
    # atomically â€” the ``chmod 600`` below stays as belt-and-suspenders.
...
# independent targets â€” both run if both are set:
#   1. TRANSCRIPT_PATH (from Claude Code) â†’ parent dir, --mode convos
#   2. MEMPAL_DIR â†’ --mode projects
...
# Silent: return empty JSON to not block. "decision": "allow" is invalid â€”
